...
def add_line_to_plane(x, y, line, plane, total_steps)
  plane[x] = {} if plane[x].nil?
  plane[x][y] = {} if plane[x][y].nil?
  plane[x][y][line] = [] if plane[x][y][line].nil?

	plane[x][y][line] << total_steps
end

{1 => line1, 2 => line2}.each do |line, movements|
	x = y = 0
	add_line_to_plane(x, y, line, plane, 0)
  num_steps = 0
    ...
    distance.times do
      ...
      num_steps += 1
			add_line_to_plane(x, y, line, plane, num_steps)
		end
	end
end
...
plane.each do |x, ys|
  ys.each do |y, lines|
    if lines.keys.count == 2 && (x != 0 || y != 0)
      intersections << {x: x, y: y, total_steps: lines.map{|k, v| v.min}.inject(0){|sum,x| sum + x }}
    end
  end
end
