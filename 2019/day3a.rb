...
def add_line_to_plane(x, y, line, plane)
...
{1 => line1, 2 => line2}.each do |line, movements|
	x = y = 0
	add_line_to_plane(x, y, line, plane)
...
plane.each do |x, ys|
  ys.each do |y, lines|
    if lines.uniq.count == 2 && (x != 0 || y != 0)
      intersections << {x: x, y: y}
    end
  end
end

distances = intersections.map{|intersection| (0 - intersection[:x]).abs + (0 - intersection[:y]).abs }

puts "Distance from origin to closest intersection: #{distances.min}"